...
def demo_query_engine():
    # 连接到真实数据库 - 请替换为您的实际连接信息
    db = MongoDBStorage(
        host="localhost",
        port=27017,
        db_name="intelligence_db",
        collection_name="intel_collection"
    )
    ...
    # 准备查询时间范围
    start_time = datetime(2023, 5, 1, tzinfo=pytz.UTC)
    end_time = datetime(2023, 5, 31, tzinfo=pytz.UTC)
    ...
    complex_results = engine.query_intelligence(
        ...
        peoples=["James Bond"],
        organizations=["SPECTRE", "MI6"],
        keywords="espionage operation",
        limit=3
    )
