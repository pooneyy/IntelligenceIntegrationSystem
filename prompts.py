...
# 核心规则
1. 输入文本若属于文学/广告/介绍类内容，**直接返回**：{"UUID":"原值"}
2. 否则按以下流程分析，所有输出必须为JSON格式
...
| PEOPLE         | 全名提取（例：["习近平"]），含头衔但需去除非姓名成分（如“张教授”只取“张”）       |
| ORGANIZATION   | 标准全称（例：["阿里巴巴集团"]），避免简写（禁用“阿里”）                     |
| EVENT_TITLE    | 18±2字核心动词短语（禁用“有关”“相关”等模糊词）                            |
| EVENT_BRIEF    | 45±5字事实描述（必须包含：主体+行为+客体）                                 |
...
- Text: 财政部2024年8月1日公告：即日起对半导体设备进口关税从10%降至5%（记者张三）
...
输出：
{
  ...
  "EVENT_TEXT": "财政部2024年8月1日公告：即日起对半导体设备进口关税从10%降至5%",
  "RATE": {
    "战略相关性":8,    // 国家级产业政策
    "国际关联度":6,    // 单边政策影响全球供应链
    "金融影响力":6,    // 改变企业资本支出规则
