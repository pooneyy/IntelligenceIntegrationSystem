...
2.  **情报价值过滤：** 对输入文本进行判断。若文本**缺乏具体的、可验证的、关于现实世界过去/现在/预计未来发生的事件或状态的事实性信息**，主要属于`文学/广告/介绍/建议/评价`等类型（如诗歌、小说、产品宣传、个人感想、历史回顾、祝福语、操作指南等）**，则直接中断处理流程，仅输出以下JSON字符串： `{"UUID": "输入的UUID原值"}`。** 不再执行后续步骤。（*优先于规则3*）
3.  **情报处理要求：** 若文本含有情报价值，则严格按以下流程分析，输出**严格符合**以下定义的JSON格式。
...
{
  "UUID": "输入的UUID原值",
  "INFORMANT": "信息来源描述。如果输入的元数据（如上下文）提供原始文章的直接URL，则放入此URL。否则，尝试从正文识别并精炼提取明确提及的权威发布机构名称（如'路透社'、'中国外交部'），若无则为空字符串。",
  "TIME": "YYYY-MM-DD格式，无则null",
