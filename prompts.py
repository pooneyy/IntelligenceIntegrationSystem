...
1.  **强制输出语言：** 无论输入语言是什么，输出内容必须为**中文**。
    > 对于常见的人名地名，尽量进行标准的中文翻译。
2.  **情报价值过滤：** 对输入文本的价值进行判断。
    > 若文本属于 **文学/广告/介绍/建议/评价/评论/历史/祝福语/指南/人文** 等无情报价值的内容类型，则直接中断处理流程，仅输出以下JSON字符串： `{"UUID": "输入的UUID原值"}`。** 不再执行后续步骤。（*优先于规则3*）
3.  **情报处理要求：** 若文本含有情报价值，则严格按以下流程分析，输出**严格符合**以下定义的JSON格式，不得输出除json外的其它内容。
    > 若文本属于 **体育/国外社会新闻** 等类型，则降低一档评分。
    > 评分要求严格，理论上80%的新闻内容应处于6分以下。
...
# 输出要求 - 有效JSON对象
{
  ...
  "EVENT_TEXT": "去除广告及无关信息后，对核心事件内容进行的**简洁、准确、完全本地化语言的翻译**。要求语言流畅自然，信息本质准确，杜绝翻译腔。",
  "RATE": { // *评分均为整数（偶数）*
    "国家政策": 0-10, // 依据国家政策标准
    "国际关系": 0-10, // 依据国际关系标准
    "政治影响": 0-10, // 依据政治影响标准
...
ANALYSIS_PROMPT = ANALYSIS_PROMPT_V2
